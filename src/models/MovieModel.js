...
class Movie {
  static async create(movieData) {
    const [result] = await db.query(
      ...
      (titulo, titulo_original, director, anio, sinopsis, imagen_url, duracion, pais, trailer_url, fecha_estreno, usuario_id) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        movieData.titulo,
        ...
        movieData.anio,
        movieData.sinopsis,
        movieData.imagen_url,
        movieData.duracion,
        movieData.pais,
        movieData.trailer_url,
        movieData.fecha_estreno,
        movieData.usuario_id,
      ]
    );
    return result.insertId;
  }

  static async findAll(limitNumber, offset) {
    const [rows] = await db.query(
      `SELECT * FROM peliculas LIMIT ? OFFSET ?`,
      [limitNumber, offset]
    );
    return rows;
  }
